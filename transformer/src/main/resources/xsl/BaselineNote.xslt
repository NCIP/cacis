...
<xsl:stylesheet version="1.1" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:cda="urn:hl7-org:v3" xmlns:trim="urn:tolven-org:trim:4.0" exclude-result-prefixes="xs cda trim">
	...
	<xsl:import href="BaselineNote_familyHistory.xslt"/>
	<xsl:import href="BaselineNote_socialHistory.xslt"/>
	<xsl:import href="BaselineNote_encounters.xslt"/>
		...
		<ClinicalDocument xsi:schemaLocation="urn:hl7-org:v3 https://ncisvn.nci.nih.gov/svn/cacis/CACIS/trunk/technical_artifacts/schema/CDA.xsd">
			...
			<component>
				<structuredBody>
					...
					<xsl:for-each select="trim:trim//trim:act/trim:relationship[@name='familyHistorys']">
					...
					<xsl:call-template name="socialHistory_section">
						<xsl:with-param name="trim_sec" select="trim:trim//trim:act/trim:relationship[@name='socialHistory']"/>
					</xsl:call-template>
					...
					<xsl:call-template name="problems_section">
						...
						<xsl:with-param name="trim_initialSymptom" select="trim:trim/trim:act/trim:relationship[@name='generalTab']/trim:act/trim:relationship[@name='initialSymptom']"/>
						...
						<xsl:with-param name="trim_phistry" select="/trim:trim/trim:act/trim:relationship[@name='priorHistory'][@enabled='true' or not(@enabled)]"/>
					</xsl:call-template>
					<!-- Medications Section -->
					...
					<xsl:if test="$menopausalStatusGroup or count(/trim:trim//trim:relationship[@name='riskFactor'])>0 or count(/trim:trim/trim:act/trim:relationship[@name='ROS'])>0">
						<component>
							<section>
					...
					<!-- Encounters -->
					<xsl:call-template name="encounters_section">
						<xsl:with-param name="trim_billing" select="/trim:trim/trim:act/trim:relationship[@name='billing']"/>
						<xsl:with-param name="trim_np" select="/trim:trim/trim:act/trim:relationship[@name='NP']"/>
						<xsl:with-param name="trim_md" select="/trim:trim/trim:act/trim:relationship[@name='MD']"/>
						<xsl:with-param name="trim_dictation" select="/trim:trim/trim:act/trim:relationship[@name='dictation']"/>
					</xsl:call-template>
					<!-- Plan of Care -->
	...
	<xsl:template name="baseline_cdaHeader">
		...
		<code code="28636-9" codeSystem="2.16.840.1.113883.6.1" displayName="Initial evaluation note"/>
