...
<xsl:stylesheet version="1.1" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:cda="urn:hl7-org:v3" xmlns:trim="urn:tolven-org:trim:4.0" exclude-result-prefixes="xs cda trim">
			...
			<component>
				<structuredBody>
					...
					<xsl:call-template name="problems_section">
						...
						<xsl:with-param name="trim_surgicalComplications" select="trim:trim/trim:act/trim:relationship[@name='surgicalComplications']"/>
					</xsl:call-template>
					...
					<xsl:if test="$menopausalStatusGroup[trim:act/trim:relationship/trim:act/trim:observation/trim:value[trim:label='Month']/trim:ST!=''] or count(/trim:trim//trim:relationship[@name='riskFactor'])>0 or count(/trim:trim//trim:relationship[@name='ROS'][trim:act/trim:relationship[@name='rosObj'][not(@enabled) or @enabled='true']])>0">
						<component>
							<section>
	...
	<xsl:template name="baseline_cdaHeader">
		...
		<code code="34848-2" codeSystem="2.16.840.1.113883.6.1" displayName="Surgery Evaluation and management note"/>
