<xsl:stylesheet version="1.1" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="procedure_section">
    ...
    <xsl:param name="trim_pathProcedures"/>
    <!-- follow up form radiation-->
    <xsl:param name="trim_radtherp"/>
    <xsl:param name="trim_radsites"/>

    <xsl:if
      test="$trim_menopausalStatus or $trim_breastCancerSurgeries or $trim_xrt or $trim_chemoBiologic or $trim_studies or $trim_rightStudies 
		  or $trim_priorProcedures or $trim_pathProcedures or $trim_radtherp or $trim_radsites">
      <component>
        <section>
          ...
          <xsl:if
            test="$trim_menopausalStatus and $trim_menopausalStatus//trim:act/trim:relationship[@name='bilateralOophorectomy']/trim:act/trim:observation/trim:value/trim:CE/trim:displayName='No'">
            <xsl:if
              test="/trim:trim//trim:relationship[@name='riskFactor']/trim:act/trim:relationship[@name='surgery']/trim:act/trim:relationship[@name='oophorectomy']/trim:act/trim:observation/trim:value/trim:SETCE/trim:displayName">
              <xsl:variable name="ooph"
                select="/trim:trim//trim:relationship[@name='riskFactor']/trim:act/trim:relationship[@name='surgery']/trim:act/trim:relationship[@name='oophorectomy']"/>
          ...
          <xsl:if test="$trim_pathProcedures">
            <xsl:for-each select="$trim_pathProcedures/trim:act/trim:relationship[@enabled='true' or not(@enabled)]">
              <xsl:call-template name="path_procedures">
                <xsl:with-param name="trim_relationship" select="current()"/>
              </xsl:call-template>
            </xsl:for-each>
          </xsl:if>

          <xsl:if test="$trim_radtherp and $trim_radsites">
            <xsl:for-each select="$trim_radsites/trim:act">
              <xsl:call-template name="procedures_radTherpy">
                <xsl:with-param name="radsite_act" select="current()"/>
                <xsl:with-param name="radth_act" select="$trim_radtherp/trim:act"/>
              </xsl:call-template>
            </xsl:for-each>
          </xsl:if>
        </section>
  ...
  <xsl:template name="path_procedures">
    <xsl:param name="trim_relationship"/>
    <xsl:variable name="proObs" select="$trim_relationship/trim:act/trim:observation"/>
    <xsl:if test="$proObs">
      <entry typeCode="DRIV">
        <procedure classCode="PROC" moodCode="EVN">
          ...
          <id nullFlavor="NI"/>
          <xsl:variable name="procNm" select="$proObs/trim:value[trim:label='Procedure']/trim:ST"/>
          <code>
            ...
            <qualifier>
              ...
              <value>
                <xsl:call-template name="translateCode">
                  <xsl:with-param name="noXSI" select="'true'"/>
                  <xsl:with-param name="trim_dispNm" select="$proObs/trim:value[trim:label='Laterality']/trim:ST"/>
                </xsl:call-template>
              </value>
            </qualifier>
          </code>
          ...
          <xsl:variable name="effcDt">
            <xsl:call-template name="format-date">
              <xsl:with-param name="theDate" select="$proObs/trim:value[trim:label='Date']/trim:ST"/>
            </xsl:call-template>
          </xsl:variable>
          <effectiveTime>
            <xsl:attribute name="value">
              <xsl:value-of select="$effcDt"/>
            </xsl:attribute>
          </effectiveTime>
          <targetSiteCode>
            <xsl:call-template name="translateCode">
              <xsl:with-param name="noXSI" select="'true'"/>
              <xsl:with-param name="trim_dispNm" select="$proObs/trim:value[trim:label='Location']/trim:ST"/>
            </xsl:call-template>
          </targetSiteCode>
          <!-- 
        <xsl:variable name="proVal">
          <xsl:if test="string-length($proObs/trim:value[trim:label='Nodes Positive']/trim:ST)>0">
            <xsl:text>Nodes Positive: </xsl:text>
            <xsl:value-of select="$proObs/trim:value[trim:label='Nodes Positive']/trim:ST"/>  
          </xsl:if>
        </xsl:variable>
        <xsl:if test="string-length($proVal)>0">
          <value xsi:type="ST">
              <xsl:value-of select="$proVal"/>
          </value>  
        </xsl:if>
        -->
        </procedure>
      ...
      <procedure classCode="PROC" moodCode="EVN">
            ...
            <value>
              <xsl:call-template name="translateCode">
                ...
                <xsl:with-param name="trim_dispNm"
                  select="$trim_relationship/trim:act/trim:observation/trim:value[trim:label='Laterality']/trim:ST"/>
              </xsl:call-template>
            </value>
          </qualifier>
        </code>
        ...
        <targetSiteCode>
          <xsl:call-template name="translateCode">
            <xsl:with-param name="noXSI" select="'true'"/>
            <xsl:with-param name="trim_dispNm"
              select="$trim_relationship/trim:act/trim:observation/trim:value[trim:label='Location']/trim:ST"/>
          </xsl:call-template>
        </targetSiteCode>
  ...
  <xsl:template name="procedures_procedures">
    ...
    <xsl:variable name="locVal"
      select="$trim_relationship/trim:act/trim:relationship[@name='laterality']/trim:act/trim:relationship[@name='location']/trim:act/trim:observation/trim:value/trim:CE/trim:displayName"/>
          ...
          <low>
            <xsl:choose>
              <xsl:when test="$trim_relationship/trim:act/trim:relationship/trim:act/trim:effectiveTime">
                <xsl:attribute name="value">
                  <xsl:value-of select="$trim_relationship/trim:act/trim:relationship/trim:act/trim:effectiveTime/trim:TS/trim:value"/>
                </xsl:attribute>
              </xsl:when>
              <xsl:otherwise>
              ...
              <effectiveTime>
                <xsl:choose>
                  <xsl:when test="trim:act/trim:effectiveTime">
                    <xsl:attribute name="value">
                      <xsl:value-of select="trim:act/trim:effectiveTime/trim:TS/trim:value"/>
                    </xsl:attribute>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:attribute name="nullFlavor">UNK</xsl:attribute>
      ...
      <procedure classCode="PROC" moodCode="EVN">
        ...
        <code>
          <xsl:call-template name="translateCode">
            <xsl:with-param name="noXSI" select="'true'"/>
            <xsl:with-param name="trim_dispNm"
              select="$trim_chemoBiologic/trim:act/trim:observation/trim:value[trim:label='Regimen']/trim:ST"/>
          </xsl:call-template>
        </code>
        <text>
          <reference>
            <xsl:attribute name="value">
              <xsl:value-of select="generate-id($trim_chemoBiologic)"/>
            </xsl:attribute>
          </reference>
        </text>
        <statusCode code="completed"/>
        <effectiveTime xsi:type="IVL_TS">
          ...
          <low>
            <xsl:attribute name="value">
              <xsl:call-template name="format-date">
                <xsl:with-param name="theDate" select="$trim_chemoBiologic/trim:act/trim:observation/trim:value[trim:label='Start']/trim:ST"
                />
              </xsl:call-template>
            </xsl:attribute>
          </low>
          <high>
            <xsl:attribute name="value">
              <xsl:call-template name="format-date">
                <xsl:with-param name="theDate" select="$trim_chemoBiologic/trim:act/trim:observation/trim:value[trim:label='End']/trim:ST"/>
              </xsl:call-template>
            </xsl:attribute>
          </high>
        </effectiveTime>
        <xsl:for-each
          select="$trim_chemoBiologic/trim:act/trim:observation/trim:value[trim:label='Episode' or trim:label='D/C Reason' or trim:label='Cycles Given']">
          <entryRelationship typeCode="REFR">
            <observation classCode="OBS" moodCode="EVN" negationInd="false">
  ...
  <xsl:template name="procedures_radTherpy">
    <xsl:param name="radth_act"/>
    <xsl:param name="radsite_act"/>
    <xsl:variable name="effTm_low" select="$radth_act/trim:effectiveTime/trim:IVL_TS/trim:low/trim:TS/trim:value"/>
    <xsl:variable name="effTm_high" select="$radth_act/trim:effectiveTime/trim:IVL_TS/trim:high/trim:TS/trim:value"/>
    <xsl:variable name="trim_site">
      <xsl:value-of
        select="$radsite_act/trim:relationship[@name='site']/trim:act/trim:observation/trim:value[trim:label='Site']/trim:CE/trim:displayName"/>
      <xsl:if
        test="$radsite_act/trim:relationship[@name='site']/trim:act/trim:observation/trim:value[trim:label='Site']/trim:CE/trim:displayName = 'Other'">
        <xsl:text>:</xsl:text>
        <xsl:value-of
          select="$radsite_act/trim:relationship[@name='site']/trim:act/trim:observation/trim:value[trim:label='Other Site']/trim:ST"/>
      </xsl:if>
    </xsl:variable>
    <xsl:variable name="trim_laterality"
      select="$radsite_act/trim:relationship[@name='laterality']/trim:act/trim:observation/trim:value[trim:label='Laterality']/trim:CE/trim:displayName"/>
    <xsl:variable name="trim_dosePer"
      select="$radsite_act/trim:relationship[@name='dose']/trim:act/trim:observation/trim:value[trim:label='Dose per FX(cGy)']/trim:PQ"/>
    <xsl:variable name="trim_totalDose"
      select="$radsite_act/trim:relationship[@name='totalDose']/trim:act/trim:observation/trim:value/trim:PQ"/>
    <xsl:variable name="trim_totalNum"
      select="$radsite_act/trim:relationship[@name='total']/trim:act/trim:observation/trim:value[trim:label='Total # FX']/trim:PQ"/>
    <entry typeCode="DRIV">
      <procedure classCode="PROC" moodCode="EVN">
        <!-- procedure template -->
        <templateId root="2.16.840.1.113883.10.20.1.29" assigningAuthorityName="CCD"/>
        ...
        <templateId root="2.16.840.1.113883.3.88.11.83.17" assigningAuthorityName="HITSP"/>
        <id nullFlavor="NI"/>
        <code code="108290001" displayName="radiation oncology AND/OR radiotherapy" codeSystem="2.16.840.1.113883.6.96"
          codeSystemName="SNOMED CT">
          <originalText>Radiation therapy; Site: <xsl:value-of select="$trim_site"/>; laterality: <xsl:value-of select="$trim_laterality"/>
          </originalText>
          <!-- site -->
          <qualifier>
            <name>
        ...
        <text>
          <reference>
            <xsl:attribute name="value">
              <xsl:value-of select="generate-id($radsite_act)"/>
            </xsl:attribute>
          </reference>
        </text>
        <statusCode code="completed"/>
        ...
        <xsl:if test="$trim_dosePer/trim:value != ''">
          <entryRelationship typeCode="REFR">
            <observation classCode="OBS" moodCode="EVN" negationInd="false">
              ...
              <templateId root="2.16.840.1.113883.3.88.11.83.15.1" assigningAuthorityName="HITSP C83"/>
              <id nullFlavor="NI"/>
              <code xsi:type="CD" code="228815006" displayName="incident radiation dose" codeSystem="2.16.840.1.113883.6.96"
                codeSystemName="SNOMED CT">
              ...
              <text>
                <reference>
                  <xsl:attribute name="value">
                    <xsl:value-of select="generate-id($trim_dosePer)"/>
                  </xsl:attribute>
                </reference>
              </text>
              <statusCode code="completed"/>
              ...
              <value xsi:type="PQ" unit="gCy/{{fraction}}">
                <xsl:attribute name="value">
                  <xsl:value-of select="$trim_dosePer/trim:value"/>
                </xsl:attribute>
              </value>
            </observation>
          </entryRelationship>
        </xsl:if>
        <xsl:if test="$trim_totalNum/trim:value != ''">
          <entryRelationship typeCode="REFR">
            <observation classCode="OBS" moodCode="EVN" negationInd="false">
              <templateId root="2.16.840.1.113883.10.20.1.31" assigningAuthorityName="CCD"/>
              <templateId root="1.3.6.1.4.1.19376.1.5.3.1.4.13" assigningAuthorityName="IHE PCC"/>
              <templateId root="2.16.840.1.113883.3.88.11.83.15.1" assigningAuthorityName="HITSP C83"/>
              <id nullFlavor="NI"/>
              <code xsi:type="CD" code="228862004" displayName="number of fractions" codeSystem="2.16.840.1.113883.6.96"
                codeSystemName="SNOMED CT">
              ...
              <text>
                <reference>
                  <xsl:attribute name="value">
                    <xsl:value-of select="generate-id($trim_totalNum)"/>
                  </xsl:attribute>
                </reference>
              </text>
              <statusCode code="completed"/>
              ...
              <value xsi:type="INT">
                <xsl:attribute name="value">
                  <xsl:value-of select="$trim_totalNum/trim:value"/>
                </xsl:attribute>
              </value>
            </observation>
          </entryRelationship>
        </xsl:if>
        <xsl:if test="$trim_totalDose/trim:value != ''">
          <entryRelationship typeCode="REFR">
            <observation classCode="OBS" moodCode="EVN" negationInd="false">
              <templateId root="2.16.840.1.113883.10.20.1.31" assigningAuthorityName="CCD"/>
              <templateId root="1.3.6.1.4.1.19376.1.5.3.1.4.13" assigningAuthorityName="IHE PCC"/>
              ...
              <text>
                <reference>
                  <xsl:attribute name="value">
                    <xsl:value-of select="generate-id($trim_totalDose)"/>
                  </xsl:attribute>
                </reference>
              </text>
              <statusCode code="completed"/>
              ...
              <value xsi:type="PQ" unit="gCy{total}">
                <xsl:attribute name="value">
                  <xsl:value-of select="$trim_totalDose/trim:value"/>
                </xsl:attribute>
              </value>
            </observation>
          </entryRelationship>
  ...
  <xsl:template name="procedures_xrt">
    ...
    <entry typeCode="DRIV">
      <procedure classCode="PROC" moodCode="EVN">
        ...
        <code code="108290001" displayName="radiation oncology AND/OR radiotherapy" codeSystem="2.16.840.1.113883.6.96"
          codeSystemName="SNOMED CT">
          <originalText>XRT; Site: <xsl:value-of select="$trim_site"/>; Episode: <xsl:value-of select="$trim_episode"/>; laterality:
              <xsl:value-of select="$trim_laterality"/>
          </originalText>
          <!-- site -->
          <qualifier>
            <name>
        ...
        <text>
          <reference>
            <xsl:attribute name="value">
              <xsl:value-of select="generate-id($trim_xrt)"/>
            </xsl:attribute>
          </reference>
        </text>
        <statusCode code="completed"/>
        ...
        <xsl:if test="$trim_dosePer/trim:ST != ''">
          <entryRelationship typeCode="REFR">
            <observation classCode="OBS" moodCode="EVN" negationInd="false">
              ...
              <value xsi:type="PQ" unit="gCy/{{fraction}}">
                <xsl:attribute name="value">
                  <xsl:value-of select="$trim_dosePer/trim:ST"/>
                </xsl:attribute>
              </value>
            </observation>
          </entryRelationship>
        ...
        <xsl:if test="$trim_totalNum/trim:ST != ''">
          <entryRelationship typeCode="REFR">
            <observation classCode="OBS" moodCode="EVN" negationInd="false">
              ...
              <value xsi:type="INT">
                <xsl:attribute name="value">
                  <xsl:value-of select="$trim_totalNum/trim:ST"/>
                </xsl:attribute>
              </value>
            </observation>
          </entryRelationship>
        ...
        <xsl:if test="$trim_totalDose/trim:ST != ''">
          <entryRelationship typeCode="REFR">
            <observation classCode="OBS" moodCode="EVN" negationInd="false">
              ...
              <value xsi:type="PQ" unit="gCy{total}">
                <xsl:attribute name="value">
                  <xsl:value-of select="$trim_totalDose/trim:ST"/>
                </xsl:attribute>
              </value>
            </observation>
          </entryRelationship>
  ...
  <xsl:template name="procedures_breastCancerSurgery">
    ...
    <xsl:param name="trim_episodicity"
      select="$trim_entry/trim:act/trim:observation/trim:value[trim:label='Episode']/trim:CE/trim:displayName"/>
    <entry>
      <procedure classCode="PROC" moodCode="EVN">
        <templateId root="2.16.840.1.113883.10.20.1.29" assigningAuthorityName="CCD"/>
        <templateId root="1.3.6.1.4.1.19376.1.5.3.1.4.19" assigningAuthorityName="IHE"/>
        ...
        <code xsi:type="CD" codeSystem="2.16.840.1.113883.6.96" codeSystemName="SNOMED CT" nullFlavor="OTH">
          <originalText> Breast cancer surgery <xsl:value-of select="$trim_episodicity"/>; Laterality/Location: <xsl:value-of
              select="$trim_laterality"/>; Episode: <xsl:value-of select="$trim_episodicity"/>
          </originalText>
          <!-- laterality -->
          <qualifier>
            <name>
        ...
        <effectiveTime>
          <xsl:choose>
            <xsl:when test="string-length($trim_effectiveTime)>0">
              <xsl:attribute name="value">
                <xsl:value-of select="$trim_effectiveTime"/>
              </xsl:attribute>
            </xsl:when>
            <xsl:otherwise>
              <xsl:attribute name="nullFlavor">UNK</xsl:attribute>
