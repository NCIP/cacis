				...
				<observation classCode="OBS" moodCode="EVN">
						...
						<xsl:when test="starts-with($vital_label,'Prior')">
							<xsl:choose>
								<xsl:when test="parent::trim:*[$vital_value]/trim:value[trim:label='Prior2 Date']">
									<effectiveTime>
										<xsl:attribute name="value"><xsl:value-of select="parent::trim:*[$vital_value]/trim:value[trim:label='Prior2 Date']/trim:TS/trim:value"/></xsl:attribute>
									</effectiveTime>
								</xsl:when>
								<xsl:otherwise>
					...
					<xsl:choose>
						<xsl:when test="starts-with($vital_label,'Prior') or starts-with($vital_label,'Name')">
							<value>
								<xsl:attribute name="xsi:type"><xsl:value-of select="'ST'"/></xsl:attribute>
								<xsl:value-of select="$vital_value/trim:ST"/>
							</value>
						</xsl:when>
						<xsl:otherwise>
							<value>
								<xsl:attribute name="xsi:type"><xsl:value-of select="'PQ'"/></xsl:attribute>
